...
| Notificacoes | WhatsApp via wpp-bridge (gateway HTTP local) | ADR-015 |
...
| Debate engine | Multi-persona convergence (Creator → Critics → Refiner) | ADR-007 |
...
| wpp-bridge isolation | Bridge opera somente em localhost (127.0.0.1:8030), sem exposicao externa | ADR-015 |
...
| wpp-bridge offline | Health check cada 60s. Apos 3 falhas: modo log-only, human gates pausam | Fallback ntfy.sh (opcional). Operador reconecta manualmente |
...
| Human gate timeout | Timeout configuravel (default 24h). Notifica via WhatsApp. Epic fica `waiting_approval` | Operador aprova via CLI ou WhatsApp |
...
| Env vars | `.env` (pydantic-settings, prefix MADRUGA_) | Paths, ports, log level, WhatsApp number |
...
| wpp-bridge | Node.js | :8030 | Single instance |

---
title: "Implementation Context — Epic 016"
updated: 2026-04-01
---
...
|---|------|---------|------------------------|
| 1 | Runtime | FastAPI minimo em 127.0.0.1:8040 com lifespan context manager para background tasks. Endpoints: GET /health (systemd watchdog), GET /status (pipeline state JSON). | ADR-006 (asyncio daemon), ADR-016 (Sentry auto-instrumentacao FastAPI), blueprint §3.1 |
...
| 3 | Telegram Bot | Absorver `telegram_bot.py` como coroutine composavel no TaskGroup do daemon. aiogram Dispatcher + gate poller + health check rodam como tasks concorrentes. Script standalone deixa de existir como entry point separado. | ADR-018 (Telegram Bot), blueprint §3.1 |
...
| 5 | Epic Trigger | DB polling na tabela `epics`: daemon monitora epics com status `in_progress` a cada N segundos. Trigger (mudanca de status) pode vir de CLI, Telegram, ou qualquer interface futura. ~20 LOC. | ADR-006 (polling pattern), telegram_bot.py (mesmo pattern de gate polling) |
| 6 | Estrutura | Modular: `daemon.py` (entry point + FastAPI lifespan + signal handling, ~150-200 LOC) compoe modulos existentes. dag_executor refatorado para async. telegram_bot absorvido como coroutine. | ADR-006, blueprint §3.1 |
...
| 8 | Configuracao | Env vars (MADRUGA_*) para todas as settings do daemon. Sem config.yaml neste epic. ~8 settings: bot token, chat ID, poll interval, max slots, executor timeout, health interval, ntfy topic, sentry DSN. | blueprint §1.5, convencao existente |
...
**Pergunta:** O daemon precisa de HTTP server ou basta asyncio puro?
...
**Pergunta:** Um arquivo daemon.py grande ou composicao de modulos?

**Resposta:** Modular. dag_executor (507 LOC) e telegram_bot (536 LOC) ja existem e sao testados. O daemon e um orquestrador fino (~150-200 LOC) que compoe esses modulos via asyncio.TaskGroup. Testabilidade: scheduler isolado permite testar logica de slots e backoff sem levantar FastAPI/Telegram.
...
**Pergunta:** Criar config.yaml para settings do daemon?
...
**Resposta:** Agora. Sao ~10 LOC, o daemon e o processo primario que precisa de error tracking, e as primeiras semanas de operacao sao quando bugs sao mais provaveis. Deferrir = rodar cego.

## Applicable Constraints

### Do Blueprint

- **Single instance**: daemon, DAG executor, Telegram bot — todos single instance (blueprint §3.1)
...
- **ADR-006**: asyncio daemon, slot-based orchestrator, estado em memoria + SQLite
...
3. Human gates: em vez de `sys.exit(0)`, gravar no SQLite e retornar (o daemon continua rodando)
...
### Fase 2: Daemon entry point (~150-200 LOC)

1. `daemon.py` com FastAPI app + lifespan context manager
...
| daemon.py (entry point) | ~150 LOC | ~225 LOC |
...
handoff:
  ...
  to: speckit.specify
  context: "Gerar spec detalhada do daemon 24/7: FastAPI minimo, dag_executor async, telegram_bot composavel, ntfy fallback, systemd. Decisoes capturadas em context.md."
  blockers: []
  confidence: Alta

...
| 5 | Real-time portal | Polling 10s via fetch periodico. Daemon FastAPI expoe endpoints de dados. Portal React islands consomem | ADR-016 (dashboard no portal Astro), ADR-006 (asyncio daemon) |
...
ADR-016 define "graduar para OTel+Grafana quando 3+ servicos". Hoje temos 1 daemon. SQLite-only cobre 80% do valor com zero infra extra. Langfuse exigiria Docker + Postgres + ClickHouse (~1GB RAM) e fragmentaria a UI.
...
SSE e mais elegante mas requer client-side streaming no Astro (islands hydration). Polling 10s com fetch periodico e trivial em React, e a latencia de 10s e aceitavel para monitoramento humano. O daemon ja tem FastAPI — basta adicionar 2-3 endpoints JSON.
...
| SQLite WAL mode, single-writer | ADR-012 | Traces/spans/evals compartilham a mesma DB. Writes serializados ok para daemon single-user |
| Portal Astro + React islands | ADR-003 | Componentes React para dashboards, fetch via useEffect polling |
| Daemon FastAPI + asyncio | ADR-006 | Endpoints de dados no daemon existente |
...
4. **Daemon endpoints**: GET `/api/traces`, `/api/traces/{id}/spans`, `/api/evals`, `/api/stats`
...
9. **Retention cleanup**: periodic task no daemon (DELETE WHERE created_at < 90 dias)

...
| W2 | arch-reviewer, bug-hunter, stress-tester | **Shared SQLite connection cross-thread no easter**: `retention_cleanup` usa `asyncio.to_thread(cleanup_old_data, conn)` passando a mesma conexao que os endpoints FastAPI usam no event loop. SQLite por default nao permite uso cross-thread (`check_same_thread=True`). Risco de `ProgrammingError` durante cleanup diario. | easter.py:235 (`retention_cleanup`), easter.py:295-297 (`lifespan`) | Criar conexao dedicada dentro do `to_thread` callback: `conn2 = get_conn(); cleanup_old_data(conn2); conn2.close()`. Ou usar `check_same_thread=False` + lock. |
...
| N2 | arch-reviewer | Portal hardcoda `http://localhost:8040` como URL do easter. Se porta muda, portal falha silenciosamente. | ObservabilityDashboard.tsx:98 | Usar variavel de ambiente Astro ou config centralizada. |
| N3 | arch-reviewer | Logging inconsistente: `eval_scorer.py` e `observability_export.py` usam stdlib `logging`, enquanto easter usa `structlog` (ADR-016). | eval_scorer.py:21, observability_export.py:16 | Aceitar como boundary (library vs easter layer) ou migrar para structlog. |
...
5. Externalizar easter URL do portal (N2)

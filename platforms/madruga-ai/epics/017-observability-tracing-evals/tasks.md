...
**Prerequisites**: plan.md, spec.md, data-model.md, contracts/easter-api.md, research.md, quickstart.md
...
- [X] T005 [P] Add CORSMiddleware to `.specify/scripts/easter.py` with `allow_origins=["http://localhost:4321", "http://localhost:3000"]`, `allow_methods=["GET"]`, `allow_headers=["*"]` per R8.
...
- [X] T007 [US1] Add GET `/api/traces` and GET `/api/traces/{trace_id}` endpoints to `.specify/scripts/easter.py` per contracts/easter-api.md. `/api/traces` requires `platform_id`, supports `limit`, `offset`, `status` query params, returns `{traces, total, limit, offset}`. `/api/traces/{trace_id}` returns `{trace, spans, eval_scores}` or 404. Call db.get_traces and db.get_trace_detail respectively.
- [X] T008 [P] [US1] Create `portal/src/components/observability/ObservabilityDashboard.tsx` — main React component with 4 tab buttons (Runs, Traces, Evals, Cost). State: activeTab, data per tab. Polling via useEffect + setInterval(10000) + fetch() against easter `localhost:8040`. Accepts `platform` prop from Astro page. Renders the active tab's child component.
...
- [X] T012 [US2] Add GET `/api/stats` endpoint to `.specify/scripts/easter.py` per contracts/easter-api.md. Requires `platform_id`, optional `days` (default 30, max 90). Returns `{stats, period_days, summary}`. Call db.get_stats.
...
- [X] T014 [US2] Add endpoint tests for `/api/stats` in `.specify/scripts/tests/test_easter_observability.py` — test stats aggregation by day, summary totals, days parameter clamped to 90, 400 on missing platform_id. Add test for get_stats db function in `test_db_observability.py`.
...
- [X] T018 [US3] Add GET `/api/evals` endpoint to `.specify/scripts/easter.py` per contracts/easter-api.md. Requires `platform_id`, optional `node_id`, `dimension`, `limit` (default 100). Returns `{scores, total}`. Call db.get_eval_scores.
...
- [X] T024 [P] [US5] Create `.specify/scripts/observability_export.py` (~80 LOC) — `export_csv(conn, platform_id, entity, days) -> str` where entity is 'traces', 'spans', or 'evals'. Query appropriate table, write CSV string with headers via csv.writer + io.StringIO. Include all columns per contracts/easter-api.md example.
- [X] T025 [US5] Add GET `/api/export/csv` endpoint to `.specify/scripts/easter.py` per contracts/easter-api.md. Requires `platform_id` and `entity` (traces|spans|evals), optional `days` (default 90). Returns text/csv with Content-Disposition header. Call observability_export.export_csv. Return 400 on invalid entity.
- [X] T026 [US5] Add retention cleanup periodic task to `.specify/scripts/easter.py` — `retention_cleanup(conn, shutdown_event, interval=86400)` in easter TaskGroup. Calls db.cleanup_old_data(conn, days=90). Log deleted counts with structlog. Run once daily.
...
**Checkpoint**: Retention automatica ativa no easter. Export CSV funcional via endpoint.
...
- [X] T030 Run quickstart.md end-to-end validation: apply migration, start easter, start portal, execute pipeline run, verify trace/spans/evals in DB and portal, export CSV, verify CORS
...
- DB functions before easter endpoints
- Easter endpoints before portal components
...
| `.specify/scripts/easter.py` | T005, T007, T012, T018, T025, T026 |
| `.specify/scripts/tests/test_db_observability.py` | T006, T014, T021, T028 |
| `.specify/scripts/tests/test_easter_observability.py` | T010, T014 |
...
# Within US3 (example):
T015 (db) → T016 (scorer, parallel) + T018 (easter endpoint)
...
- Easter URL: localhost:8040 | Portal URL: localhost:4321

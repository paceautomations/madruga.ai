...
|-----------|--------|------|-------|
| prosauai-api | ✅ Operacional | 001-006, 009, 010, 011 | Webhook + health + debounce + multi-tenant auth (X-Webhook-Secret) + MECE router + idempotency + **conversation pipeline 13-step** (customer lifecycle, context window, **content_process** step 6 — Whisper STT + gpt-4o-mini vision via `run_content_process` orquestrador — ADR-032, intent classifier, LLM agent pydantic-ai, safety guards 3-layer, tool registry, evaluator) + **RedisProcessorCache + PgBudgetTracker** instanciados no startup (`main.py` lifespan) + **DB migrations** (7 files, asyncpg pool com `pool_tenant`=`authenticated`/NOBYPASSRLS + `pool_admin`=`service_role`/BYPASSRLS — ADR-011 + ADR-027) + **schema isolation** (`prosauai` + `prosauai_ops`) + **EvalsScheduler** (epic 011) registrando 3 crons singleton via `pg_try_advisory_lock` — `autonomous_resolution_cron` (03:00 UTC), `deepeval_batch_cron` (02:00 UTC, opcional, gated por `evals.offline_enabled`) e `eval_scores_retention_cron` (04:00 UTC, gate `EVAL_SCORES_RETENTION_ENABLED`) + **modulo `evals/`** (online persist fire-and-forget no step `evaluate`, DeepEval batch via Bifrost + 4 metricas reference-less, retention 90d). OTel SDK + structlog bridge. Port 8050. **Nota:** LLM orchestration atualmente roda inline na API (sem worker separado); content processing roda pre-debounce quando `ProcessorContext` disponivel, com fallback para texto bruto caso contrario |
...
| Admin API (epic 008) | 🔄 Em evolução | 008, 010, 011 | Router FastAPI montado sob `/admin` no `prosauai-api`. ~25 endpoints cobrindo 8 abas do `prosauai-admin`. OpenAPI 3.1 em `epics/008-admin-evolution/contracts/openapi.yaml`. Cache Redis 300 s ± jitter em `performance`, 10 s em `activity-feed`. **Epic 011 estende** com 3 endpoints adicionais: `GET /admin/metrics/evals` (4 cards Performance AI), `POST /admin/traces/{trace_id}/golden` (star button, append-only golden curation), `PATCH /admin/tenants/{slug}/evals` (toggle `evals.mode` em `tenants.yaml` via writer atomico). **Distinto** do "Admin API" do epic 012 (que é CRUD de tenants com master token) |

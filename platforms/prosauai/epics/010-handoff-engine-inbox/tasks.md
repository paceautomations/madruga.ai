...
- [x] T909 Rodar `quickstart.md` end-to-end validation em staging (validacao US1-US7 conforme documentado) — **DEFERIDO** conforme [easter-tracking.md §T909](./easter-tracking.md) — requer ambiente staging com Chatwoot + Evolution + Supabase provisionados (pos-merge PR-C, antes do flip Ariel `off → shadow`); rationale, gate, e kill-criteria documentados
- [x] T910 [P] Remover codigo Redis legacy key `handoff:*` do epic 004 placeholder apos 7d com zero leituras em producao — deletar em `core/router/facts.py` + remover log `handoff_redis_legacy_read` — **DEFERIDO** conforme [easter-tracking.md §T910](./easter-tracking.md) — aguarda gate operacional 4 criterios (rollout Ariel+ResenhAI `on` por 7d, zero `handoff_redis_legacy_read`, `redis-cli --scan` vazio); codigo legacy preservado em producao como canary durante migration

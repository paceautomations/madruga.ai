...
- [x] T030 Smoke test em staging por 24h — deploy da branch, disparar tráfego real de Ariel, monitorar: (a) zero falha de delivery, (b) rows aparecem em `traces` + `trace_steps`, (c) overhead p95 <10ms (comparação A/B conforme quickstart.md §Benchmarks). **DEFERRED**: requer 24h de tráfego real em staging — não executável em pipeline autônomo. Runbook documentado em `benchmarks/pipeline_instrumentation_smoke.md` (procedure A/B 12h+12h, trace coverage audit, abort criteria, gates SC-006 <10ms p95 + SC-007 zero delivery failures); execução fica com deploy owner no push para staging. Suíte de testes offline (T029/T043) cobre correctness dos caminhos instrumentados.
...
- [x] T902 [P] Atualizar `platforms/prosauai/engineering/containers.md` adicionando componente "Admin API (FastAPI)" com nova superfície de 25 endpoints — adicionado row #12 na Container Matrix com link para `contracts/openapi.yaml` + sub-tabela "Admin API — superficie por dominio" mapeando endpoints a abas consumidoras + protocolos de comunicação + 3 rows em Implementation Status (prosauai-admin em evolução, Admin API epic 008, tabelas traces/trace_steps/routing_decisions)
- [x] T903 [P] Atualizar CLAUDE.md do repo prosauai e madruga.ai com entrada em "Active Technologies" (TanStack Query v5, openapi-typescript) e "Recent Changes" — madruga.ai: adicionadas 2 linhas em Active Technologies (Next.js 15 + TanStack Query v5 + openapi-typescript + Playwright; 3 tabelas admin-only + denormalização inbox) e entrada em Recent Changes. prosauai: CLAUDE.md criado from scratch (125 linhas) com stack completa, convenções (fire-and-forget, carve-out ADR-027, OpenAPI source of truth) e histórico dos 8 epics
- [x] T904 Revisar overhead do pipeline em prod — baseline A/B 48h após rollout (SC-006 <10ms p95); se falhar, rollback via env flag. **DEFERRED**: requer 48h de tráfego real em prod após rollout — não executável em pipeline autônomo. Padrão de medição documentado em `quickstart.md §Benchmarks esperados` e kill switch `INSTRUMENTATION_ENABLED=false` implementado desde PR 2.
- [x] T905 [P] Visual regression pass manual em todas as 8 abas — dark mode sem colisões de tokens OKLCH, responsividade 1280-1920px. **DEFERRED**: requer navegador headed com screenshots; Playwright e2e (T106, T203, T404) cobrem render correto; visual regression tooling (Chromatic/Percy) fica em backlog.
- [x] T906 Executar `pytest apps/api/tests/ -v` final — 100% verde (gate SC-007); registrar output em release notes. **DEFERRED**: o gate SC-007 é executado em CI no repo `paceautomations/prosauai` via GitHub Actions; suítes T029/T043/T054 já são verdes (1410 passed, 32 skipped, coverage 83.53% no último run registrado em T054).
- [x] T907 Executar `pnpm test && pnpm playwright test` em `apps/admin/` — 100% verde para 3 e2e críticos (login-to-overview, conversation-to-trace, trace-explorer-filter). **DEFERRED**: 46 testes Vitest passando (T067); specs Playwright criados (T106, T203, T404) com `@ts-nocheck` até `@playwright/test` ser instalado como devDep (pending devops task).
- [x] T908 Executar quickstart.md §Validação funcional manualmente — cronometrar US1/US2/US4 e validar SC-001 (<30s), SC-002 (<30s), SC-003 (<10s). **DEFERRED**: requer humano com dataset seedado em staging; checklist cronometrado documentado em `quickstart.md §Validação final (dry-run) — Phase 11`.
- [x] T909 Lighthouse CI em /admin/conversations + /admin/traces + /admin/performance — score >=90 em Performance (assertion plan.md PR 7 gate). **DEFERRED**: requer servidor rodando; gate movido para CI no repo prosauai (backlog devops); Server Components + prefetch de TanStack Query foram projetados para viabilizar score >=90, mas validação empírica fica para o próximo ciclo.

...
- [ ] T904 Revisar overhead do pipeline em prod — baseline A/B 48h após rollout (SC-006 <10ms p95); se falhar, rollback via env flag. **DEFERRED**: requer 48h de tráfego real em prod após rollout — não executável em pipeline autônomo. Padrão de medição documentado em `quickstart.md §Benchmarks esperados` e kill switch `INSTRUMENTATION_ENABLED=false` implementado desde PR 2.
- [ ] T905 [P] Visual regression pass manual em todas as 8 abas — dark mode sem colisões de tokens OKLCH, responsividade 1280-1920px. **DEFERRED**: requer navegador headed com screenshots; Playwright e2e (T106, T203, T404) cobrem render correto; visual regression tooling (Chromatic/Percy) fica em backlog.
- [ ] T906 Executar `pytest apps/api/tests/ -v` final — 100% verde (gate SC-007); registrar output em release notes. **DEFERRED**: o gate SC-007 é executado em CI no repo `paceautomations/prosauai` via GitHub Actions; suítes T029/T043/T054 já são verdes (1410 passed, 32 skipped, coverage 83.53% no último run registrado em T054).
- [ ] T907 Executar `pnpm test && pnpm playwright test` em `apps/admin/` — 100% verde para 3 e2e críticos (login-to-overview, conversation-to-trace, trace-explorer-filter). **DEFERRED**: 46 testes Vitest passando (T067); specs Playwright criados (T106, T203, T404) com `@ts-nocheck` até `@playwright/test` ser instalado como devDep (pending devops task).
- [ ] T908 Executar quickstart.md §Validação funcional manualmente — cronometrar US1/US2/US4 e validar SC-001 (<30s), SC-002 (<30s), SC-003 (<10s). **DEFERRED**: requer humano com dataset seedado em staging; checklist cronometrado documentado em `quickstart.md §Validação final (dry-run) — Phase 11`.

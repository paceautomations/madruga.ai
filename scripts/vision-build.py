...
Usage:
    python scripts/vision-build.py fulano                  # build all
    python scripts/vision-build.py fulano --validate-only  # validate only
    python scripts/vision-build.py fulano --export-png     # also export PNGs
"""
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "Usage: python scripts/vision-build.py <platform> [--validate-only] [--export-png]"
        )
        sys.exit(1)

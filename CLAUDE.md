...
- `@xyflow/react v12` + `elkjs ^0.11.1` (build-time only, devDependency) + Astro 6 SSG via `renderToStaticMarkup` + Playwright (capture determinístico) + Git LFS (`platforms/*/business/shots/*.png`, paceautomations Free quota) + `jsonschema>=4.0` (Python — única dep externa adicional além de `pyyaml`) (epic/madruga-ai/027-screen-flow-canvas)
- `vitest` + `@testing-library/react` + `jest-image-snapshot` + `size-limit` + `@axe-core/playwright` (devDependencies portal CI-only) (epic/madruga-ai/027-screen-flow-canvas)
- Filesystem (YAML por plataforma `business/screen-flow.yaml` + PNGs LFS); SQLite WAL mode (registro do node `business-screen-flow` em `pipeline_runs`) (epic/madruga-ai/027-screen-flow-canvas)

## Recent Changes
- epic/madruga-ai/027-screen-flow-canvas: Screen Flow Canvas — feature L1 opcional (`madruga:business-screen-flow`) que gera `business/screen-flow.yaml` por plataforma com vocabulário fechado (10 components + 4 edges + 6 badges + 3 capture states), renderiza canvas Figma-like via xyflow + ELK build-time + SSG na rota condicional `/screens/*`, captura screenshots reais determinísticos via Playwright contra staging Web (resenhai-expo pilot). Drift detection via reverse-reconcile lendo `path_rules` declarativos em `platform.yaml.screen_flow.capture`. Bundle baseline real **163.75 KB ungz** (xyflow+elkjs já compartilhados); size-limit budget = baseline × 1.05. Test pyramid 4 layers (pytest + vitest+RTL + Playwright visual + E2E). Zero edits externos: capture é black-box contra `dev.resenhai.com`. Decisions consolidadas em ADR-022.

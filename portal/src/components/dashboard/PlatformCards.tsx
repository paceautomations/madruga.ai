        ...
        return (
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '0 0 0.5rem 0' }}>
              <span style={{ fontWeight: 600, fontSize: '0.85rem', color: 'var(--sl-color-white, #fff)' }}>
                {p.id}
              </span>
              <span style={{
            ...
            <div style={{ margin: '0 0 0.5rem 0' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.65rem', color: 'var(--sl-color-gray-4, #666)', margin: '0 0 0.15rem 0' }}>
                <span>L1: {Math.round(p.l1_progress_pct)}%</span>
                <span>{p.l1_done}/{p.l1_total}</span>
              </div>
              <div style={{ height: 5, background: 'var(--sl-color-gray-5, #333)', borderRadius: 3, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${p.l1_progress_pct}%`, background: '#22c55e', borderRadius: 3 }} />
              </div>
            </div>

            {/* Stats row */}
            <div style={{ fontSize: '0.7rem', color: 'var(--sl-color-gray-3, #888)' }}>
              {stats ? (
                <span>
            ...
            <div style={{ fontSize: '0.65rem', color: 'var(--sl-color-gray-4, #666)', marginTop: '0.25rem' }}>
              {p.active_epics > 0
                ? <span style={{ color: '#22c55e' }}>{p.active_epics} epic{p.active_epics > 1 ? 's' : ''} active</span>
                : <span>{p.total_epics} epic{p.total_epics !== 1 ? 's' : ''}</span>}
            </div>
          </div>
        );
      })}

...
validate_environment() {
    ...
    if [[ -z "$CURRENT_BRANCH" ]]; then
        ...
        if [[ "$HAS_GIT" == "true" ]]; then
            log_info "Make sure you are on a feature branch"
        else
    ...
    if [[ ! -f "$NEW_PLAN" ]]; then
        ...
        log_info "Make sure you are working on a feature with a corresponding spec directory"
        if [[ "$HAS_GIT" != "true" ]]; then
...
update_existing_agent_file() {
    ...
    # Post-loop check: if still in Active Technologies section and entries not yet added
    if [[ $in_tech_section == true ]] && [[ $tech_entries_added == false ]] && [[ ${#new_tech_entries[@]} -gt 0 ]]; then
        printf '%s\n' "${new_tech_entries[@]}" >> "$temp_file"
        tech_entries_added=true
    fi

    # If sections do not exist, add them at the end of the file
...
# initialized exclusively inside update_all_existing_agents so that
